...
換句話說，**它缺的不是更多幾何，是讓光「打對地方」**。所以我決定不鋪量，先把章魚燒這一站當試點，一路升到頂，再拿這套規格去套其他四站。
...
零資產也不再是限制，反而變成賣點：**這張環境是算出來的，不是下載來的**。
...
`threshold: 0.9` 是整個效果的開關：章魚燒本體再亮也到不了那個值，只有 emissive 拉爆的霓虹招牌會被撈出來做光暈。**發光變成一個「你指定誰發光」的決定，而不是全畫面糊一層。** 最後那顆 `OutputPass` 也很關鍵——後製鏈是在線性空間跑的，色彩空間轉換要留到最後一步統一做，順序錯了顏色就會偏。
...
執行上我也把重活拆給專精的分工去做：程序化建模、shader 材質、後製管線，這些各自是很深的坑，與其一個人樣樣半桶水，不如把明確界定的子任務派給對應的專精角色，主線只負責定規格、驗收、把它們縫成一個順的體驗。**縫得剛剛好，比每一塊都自己造，重要得多。**
