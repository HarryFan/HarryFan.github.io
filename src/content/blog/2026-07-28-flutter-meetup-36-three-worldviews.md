...
> 今晚回到天瓏二樓，Flutter Meetup #36。這次不是坐在台下抄筆記，是站在台上講 30 分鐘。
...
先講清楚定位：這不是 Flutter 教學。台下大多是前端工程師，有些人連 Dart 都沒碰過。我要做的不是教怎麼寫，是帶大家理解這三個框架在腦子裡是三種完全不同的世界觀，各自該用在哪。

這篇是講稿的完整版，把 30 分鐘裡因為時間壓縮掉的東西補回來。

## 為什麼是我來講這個

我很早就想做 Flutter，卡住我的從來不是它難，是 Dart。

我有正職，光把 JavaScript 寫好就佔滿了，沒有多餘心力再養一個語言。那本 Dart 的書買了放在那，一直沒拆。
...
這件事要先說明白，因為它決定了我今天能講什麼、不能講什麼：Waypoint Air 是拿來練手的，不是上線產品。所以我談的是**開發體感與世界觀，不是流量規模下的效能數據**。沒有 bundle size 表格，沒有 Lighthouse 分數。想看那個的，這場不是。
...
我在台上刻意在這段停久一點。因為這不是效能數字問題，是**開發流程被打斷**的身體記憶 —— 而這個記憶每個前端都有。
...
Wasm 這件事值得多解釋一句，因為現場有人不熟。它是一種低階的二進位格式，可以把 C、C++、Rust 這類語言編譯成瀏覽器能安全執行的模組，在沙箱裡以接近原生的速度跑。你可以想成：高效能的部分交給 Wasm，JavaScript 繼續負責跟網頁世界互動。**兩個不是互斥，是互補。**
...
官方定義很直接：熱重載把**更新後的原始碼檔案，注入到正在跑的 Dart 運行時（runtime）**。運行時拿到新的欄位跟函數之後去更新對應的類別，接著 Flutter framework **自動重建 widget 樹**，畫面就跟上了。
...
這條鏈裡**沒有一步叫「重新啟動」**。這就是為什麼路由堆疊、表單輸入、捲動位置、彈窗開著沒有，全部都在。

對前端來說，這等於 Vite HMR 的體感，只是它保的是 widget 狀態，不是模組。
...
Agentic Hot Reload 是把這個迴圈交給 AI 編碼代理人，而且不是「一次性生 code 就結束」。透過新的 Dart MCP server，代理人可以讀整個專案、就地改 code、對**正在跑的 App** 直接 hot reload，然後把現場診斷拉回來 —— widget tree、render 效能、network log、exception，然後自己再跑下一輪。

迴圈從「我改 → 我看」，變成「**它改 → 它看 → 它再改 → 我最後 approve**」。

真正厲害的地方不是 AI 會寫 code，是它第一次能對 running app 建立**秒級的除錯回饋迴圈**。而這件事對前端的意義比對 mobile 更大，因為前端本來就活在瀏覽器裡的即時回饋中。

實際用起來像這樣：我丟一句「修一下小尺寸平板橫向時訂單摘要的 overflow」，它改 widget tree、hot reload、看結果、發現 regression，再把那個也補掉。

角色也跟著變了。**我不再是每塊磚都自己搬的泥水匠，更像是在旁邊驗收、下判斷、最後簽核的建築師。**
...
過去大家不選 Flutter Web，很多時候理由是「開發起來不順」。

這個理由現在不成立了。那真正的差別在哪？在世界觀。
...
- Flutter Web 的產物是**一張畫布**。你選不到任何一個 div。

**左邊是元素，右邊是像素。**

這不是實作細節，這決定了後面所有的優點跟代價。今天要講的一切都從這裡長出來。
...
差別在**心智負擔的位置** —— 是框架幫你追，還是你負責描述當下。

Vue 幫你記；React 要你重跑；Flutter 要你描述 —— `build()` 回傳「此刻該長怎樣」，完全不管上一幀。
...
所以 Flutter 的巢狀會很深。這不是壞味道，**是它的表達方式**。
...
**Web 血統**：路由天生綁 URL，可分享、可重新整理、可上一頁。
**App 血統**：預設是頁面堆疊，URL 是後加的選配。

這是兩種血統最直接的分水嶺。在 Web 世界裡「網址會變」是預設值；在 Flutter 裡那是你要自己補的。
...
講白一點，這是**聖杯等級的野心**：不是「能跑就好」，而是三端看起來要幾乎一樣。

而這個特性，是它所有優勢的來源，也是它所有代價的來源。
...
**但生態這半補不掉。** npm 幾乎什麼都有，pub.dev 的 Web 端套件明顯薄。遇到坑，Vue / React 通常已經有人寫過答案；Flutter Web 常常要自己試 —— 而 AI 也是拿同一批資料在猜。

所以真正的成本不是「學不學得會」，是**卡住的時候有沒有人踩過**。
...
**一、這頁需要被 Google 找到嗎？**
需要 → 不要 Flutter Web。**這是否決題。**

**二、這個產品之後要上手機嗎？**
要 → Flutter Web 立刻加分。

**三、卡住的時候，有人踩過這個坑嗎？**
注意，問的不是「會不會寫」—— 那題 AI 已經幫你答了。
...
Wasm 已經 stable，roadmap 上要變成 Web 的預設。這背後的意思是：瀏覽器越來越能接受「不是只跑 JavaScript」的高效能模組，**Web 的地板正在往上抬**。

同時，agentic 開發的迴圈正在成形 —— 我那個 side project 就是這樣做完的。

這兩件事會繼續往下壓 Flutter Web 的門檻。

而語言不再是門檻之後，選型的重點會整個往上移：**從「我會什麼」變成「我判斷得準不準」。**

所以我今天講的判斷有保存期限，明年這三個問題的答案可能不一樣。再大膽一點想：如果有一天 AI 能把 canvas 畫面即時逆譯成可索引、可讀屏的語意結構，那今天的死穴可能會變成明天的突破口。
...
2. 真正的差別是 **DOM 還是 Canvas** —— 這決定了 SEO、無障礙、跨平台三件事。
3. 先問「使用者從哪裡來」，再選框架。
...
投影片裡那三扇門，我一扇都沒推開，只是拿著量尺在量門框。先量再選，沒有最好的。

謝謝天瓏、謝謝 Flutter Taipei 跟 GDG Taipei，也謝謝當天留下來 Q&A 的每一位。
